//
// FILENAME.
//      BopRemovableDisk.c - SecureCore Technology(TM) Boot Option Protocol for Fixed Disk.
//
...
static PCHAR16 mRemovableEndNodeDevicePaths[] = {
    L"USB(0xFF,0xFF)",
    L"Unit(0xFF)",
};

static UINTN mRemovableEndNodeDevicePathsSize = sizeof (mRemovableEndNodeDevicePaths) / sizeof (mRemovableEndNodeDevicePaths[0]);

//
// Connect list of the platform.
//

static BOOT_MANAGER_CONNECTION_DEVICE mRemovableConnectListArray[] = {
  CONFIG_Mb3RemovableConnectList
};

static UINTN mRemovableConnectListArraySize =
  sizeof (mRemovableConnectListArray) / sizeof (mRemovableConnectListArray[0]) - 1;
...
EFI_STATUS
Mb3RemovableXSearchDevicePaths (
...
VOID
Mb3RemovableSaveDeviceRecord (
  VOID
  );

VOID
Mb3RemovableFullScanPerformed (
  VOID
  );

EFI_STATUS
Mb3RemovableXInsertDeviceRecord (
  IN UINT8 ControllerIndex,
  IN UINT8 SearchNodeIndex,
...
EFI_STATUS
Mb3RemovableXGetChildren (
  IN PMB3_DEVICE_RECORD Record,
  OUT PSCT_BOOT_OPTION_NODE *ChildListHead
  );

PCHAR16
Mb3RemovableCreateDeviceDescription (
  IN EFI_DEVICE_PATH_PROTOCOL *DevicePath
  );

BOOLEAN
Mb3RemovableIsDevicePathRemovableDisk (
  IN EFI_DEVICE_PATH_PROTOCOL *DevicePath
  );
...
//
// FUNCTION NAME.
//      Mb3RemovableGetDevicePaths - Execute the policy for Mb3Removable Boot Option.
//
// FUNCTIONAL DESCRIPTION.
  ...
  *NumberOfDevicePaths = 0;
  ...
  if ((DeviceRecordCacheSize != 0) || (SavedDeviceRecordCacheSize > 0)) {
    ...
    for (Count = 0; Count < DeviceRecordsCount; ++Count) {
      ...
      dp = NULL;
      n = 0;
      Status = Mb3RemovableXGetDevicePaths (i, j, Record, &dp, &n);
      DPRINTF_MB3 ("Mb3RemovableXGetDevicePaths returned %r, %d\n", Status, n);
      if ((EFI_ERROR (Status)) || (n == 0)) {
        DPRINTF_MB3 ("Continue.\n");
        continue;
      }

      DISPLAY_DEVICE_PATH_ARRAY (dp, n, L"  Mb3RemovableX:");
  ...
  for (i = 0; i < mRemovableConnectListArraySize; i++) {
    for (j = 0; j < SearchNodeDevicePathArraySize; ++j) {
      dp = NULL;
      n = 0;
      Status = Mb3RemovableXGetDevicePaths (i, j, NULL, &dp, &n);
      DPRINTF_MB3 ("Mb3RemovableXGetDevicePaths returned %r, %d\n", Status, n);
      if ((EFI_ERROR (Status)) || (n == 0)) {
        DPRINTF_MB3 ("Continue.\n");
        continue;
  ...
  )
{
  ...
  for (i = 0; i < mRemovableConnectListArraySize; i++) {
    if (mRemovableConnectListArray[i].TextDevicePath == NULL) {
      continue;
    }
    ...
    for (j = 0; j < SearchNodeDevicePathArraySize; ++j) {
      ...
      if (!EFI_ERROR (Status)) {
        p = dp;
        for (k = 0; k < n; ++k) {
          Mb3RemovableXInsertDeviceRecord (i, j, p);
          p = NextDevicePath (p);
        }
      }
      SafeFreePool (dp);
  ...
  )
{
  ...
  *ChildListHead = NULL;
  ...
  for (i = 0; i < DeviceRecordCacheSize; ++i) {
    DPRINTF_MB3 ("Mb3RemovableXGetChildren loop 0x%x.\n", i);
    Status = Mb3RemovableXGetChildren (&(DeviceRecordCache[i]), ChildListHead);
    DPRINTF_MB3 ("Mb3RemovableXGetChildren returned %r.\n", Status);
  }
...
SCT_BOOT_OPTION_PROTOCOL mMb3RemovableBootOptionProtocol = {
...
//
// FUNCTION NAME.
//      Mb3RemovableSaveDeviceRecord - Save DeviceRecordCache to variable.
//
// FUNCTIONAL DESCRIPTION.
...
//
// FUNCTION NAME.
//      Mb3RemovableFullScanPerformed - Callback when a full scan has been performed.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  DPRINTF_MB3 ("Initialize gConnectedList of size 0x%x.\n", mRemovableConnectListArraySize);

  gConnectedList = AllocateZeroPool (mRemovableConnectListArraySize * sizeof (BOOLEAN));
  DPRINTF_MB3 (
    "Allocated gConnectedList of size 0x%x @0x%x.\n",
    mRemovableConnectListArraySize * sizeof (BOOLEAN),gConnectedList
    );
  ...
  SearchNodeDevicePathArraySize = mRemovableEndNodeDevicePathsSize;
  ...
  for (i = 0; i < mRemovableEndNodeDevicePathsSize; ++i) {
    SearchNodeDevicePathArray[i] = mRemovableEndNodeDevicePaths[i];
...
//
// FUNCTION NAME.
//      InitializeBopRemovableDiskMb3 - Initialize the BopMb3Removable module.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  Status = gBS->InstallMultipleProtocolInterfaces (
                  &ImageHandle,
                  &gMb3RemovableDiskBootOptionProtocolGuid,
                  &mMb3RemovableBootOptionProtocol,
                  NULL
                  );
  return Status;
} // InitializeBopRemovableDiskMb3
...
//
// FUNCTION NAME.
//      Mb3RemovableXInsertDeviceRecord - Insert a DeviceRecord into cache.
//
// FUNCTIONAL DESCRIPTION.
...
//
// FUNCTION NAME.
//      Mb3RemovableXGetDevicePaths - Get device path from give device path pair.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  //
  // Connect all the device paths associated with this Mb3Removable Device.
  //

  Status = Mb3RemovableXSearchDevicePaths (ControllerIndex, SearchNodeIndex, &dp, &n);
  DPRINTF_MB3 ("Mb3RemovableXSearchDevicePaths returned %r, %d device paths.\n", Status, n);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  DISPLAY_DEVICE_PATH_ARRAY (dp, n, L"  Mb3RemovableXSearchDevicePaths:");
  ...
  *NumberOfDevicePaths = 0;
  ...
  for (i = 0; i < n; i++, p = NextDevicePath (p)) {
    ...
    if (!Mb3RemovableIsDevicePathRemovableDisk (p)) {
      DPRINTF_MB3 ("DevicePath doesn't represent a removable disk...\n");
      continue;
    }
...
//
// FUNCTION NAME.
//      Mb3RemovableXSearchDevicePaths - Get device path from give device path pair.
//
// FUNCTIONAL DESCRIPTION.
//      This function searchs for all device paths to find an applicable matching 
//      given device pair. This function wraps this common action.
//
// ENTRY PARAMETERS.
//      ControllerIndex        - The index of the parent device path.
//      SearchNodeIndex - The index of the search node device path.
  ...
  )
{
  ...
  if (gConnectedList[ControllerIndex] == FALSE) {
    ...
    Status = SctBdsLibConnectDevices (&(mRemovableConnectListArray [ControllerIndex]));
    if (EFI_ERROR (Status)) {
  ...
  ShadowOproms (&(mRemovableConnectListArray[ControllerIndex]));

  //
  // Search for devices that match this Mb3Removable criteria.
  //

  PathToSearch.ParentDevicePathText = mRemovableConnectListArray[ControllerIndex].TextDevicePath;
  ...
  DPRINTF_MB3 ("Returned %r, %d entries.\n", Status, TempN);
  return Status;
} // Mb3RemovableXSearchDevicePaths

//
// FUNCTION NAME.
//      Mb3RemovableXGetChildren - Append Child Nodes to a Linked List.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  dp = NULL;
  n = 0;
  Status = Mb3RemovableXGetDevicePaths (
             Record->ControllerIndex,
             Record->SearchNodeIndex,
             Record,
             &dp,
  ...
  DPRINTF_MB3 ("Mb3RemovableXGetDevicePaths returned %r, %d entries.\n", Status, n);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  for (i = 0; i < n; i++, p = NextDevicePath (p)) {
    ...
    *q = AllocateZeroPool (sizeof (SCT_BOOT_OPTION_NODE));
    ...
    DPRINTF_MB3 ("New BootOptionNode 0x%x:0x%x\n", q, *q);
    ...
    (*q)->Description = Mb3RemovableCreateDeviceDescription (p);
...
//
// FUNCTION NAME.
//      Mb3RemovableCreateDeviceDescription - Create a string based on a device path.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  DPRINTF_MB3 ("  HandleProtocol (DiskInfo): %r\n", Status);
  if (EFI_ERROR (Status)) {
  ...
  if (CompareGuid (&DiskInfo->Interface, &gEfiDiskInfoIdeInterfaceGuid) ||
    ...
    DPRINTF_MB3 ("  DiskInfo->Identify: %r\n", Status);
    if (EFI_ERROR (Status)) {
      goto CreateDeviceDesc_blockio;
    }
...
//
// FUNCTION NAME.
//      Mb3RemovableIsDevicePathRemovableDisk - Check to see if a device path is an fixed disk.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (BbsBcv.DeviceType == BBS_FLOPPY) {
      return TRUE;
    }
    else if (BbsBcv.DeviceType == BBS_HARDDISK) {
      return TRUE;
    }
    else if (BbsBcv.DeviceType == BBS_CDROM) {
      return TRUE;
    }
    else {
      return FALSE;
  ...
  if (IsRemovableMediaDevicePath (DevicePath, NULL)) {
    ...
    DPRINTF_MB3 (
        "  GetUsbBbsDeviceTypeFromDevicePath:\n"
        "    DeviceType = 0x%02x.\n"
        "    Status = %r.\n",
        DeviceType,
        Status
    ...
    // If status with error or DeviceType not in [BBS_FLOPPY,BBS_HARDDISK,BBS_CDROM]
    // set, return FALSE
    if (EFI_ERROR (Status) || 
