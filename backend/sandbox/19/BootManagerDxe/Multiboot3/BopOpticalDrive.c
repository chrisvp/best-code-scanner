//
// FILENAME.
//      BopOpticalDrive.c - SecureCore Technology(TM) Boot Option Protocol for Fixed Disk.
//
...
static PCHAR16 mCdEndNodeDevicePaths[] = {
...
static UINTN mCdEndNodeDevicePathsSize = sizeof (mCdEndNodeDevicePaths) / sizeof (mCdEndNodeDevicePaths[0]);

//
// Length of the template.
//

#define BOP_SATA_DEVICE_LENGTH 16
static PCHAR16 SataDevicePathTemplate = L"Sata(%x,0,0)";
...
static BOOT_MANAGER_CONNECTION_DEVICE mCdConnectListArray[] = {
  CONFIG_Mb3CdConnectList
};

static UINTN mCdConnectListArraySize =
  sizeof (mCdConnectListArray) / sizeof (mCdConnectListArray[0]) - 1;
...
PCHAR16
Mb3CdCreateDeviceDescription (
  IN EFI_DEVICE_PATH_PROTOCOL *DevicePath
  );

BOOLEAN
Mb3CdIsDevicePathOpticalDrive (
  IN EFI_DEVICE_PATH_PROTOCOL *DevicePath
  );
...
//
// FUNCTION NAME.
//      Mb3CdGetDevicePaths - Execute the policy for Mb3Cd Boot Option.
//
// FUNCTIONAL DESCRIPTION.
  ...
  *NumberOfDevicePaths = 0;
  ...
  if ((DeviceRecordCacheSize != 0) || (SavedDeviceRecordCacheSize > 0)) {
    ...
    for (Count = 0; Count < DeviceRecordsCount; ++Count) {
      ...
      dp = NULL;
      n = 0;
      Status = Mb3CdXGetDevicePaths (i, j, Record, &dp, &n);
      DPRINTF_MB3 ("  Mb3CdXGetDevicePaths returned %r, %d\n", Status, n);
      if ((EFI_ERROR (Status)) || (n == 0)) {
        DPRINTF_MB3 ("  Continue.\n");
        continue;
      }

      DISPLAY_DEVICE_PATH_ARRAY (dp, n, L"  Mb3CdX:");
  ...
  for (i = 0; i < mCdConnectListArraySize; i++) {
    for (j = 0; j < SearchNodeDevicePathArraySize; ++j) {
      dp = NULL;
      n = 0;
      Status = Mb3CdXGetDevicePaths (i, j, NULL, &dp, &n);
      DPRINTF_MB3 ("  Mb3CdXGetDevicePaths returned %r, %d\n", Status, n);
      if ((EFI_ERROR (Status)) || (n == 0)) {
        DPRINTF_MB3 ("  Continue.\n");
        continue;
  ...
  )
{
  ...
  for (i = 0; i < mCdConnectListArraySize; i++) {
    if (mCdConnectListArray[i].TextDevicePath == NULL) {
      continue;
    }
    ...
    for (j = 0; j < SearchNodeDevicePathArraySize; ++j) {
      ...
      if (!EFI_ERROR (Status)) {
        p = dp;
        for (k = 0; k < n; ++k) {
          Mb3CdXInsertDeviceRecord (i, j, p);
          p = NextDevicePath (p);
        }
      }
      SafeFreePool (dp);
  ...
  )
{
  ...
  *ChildListHead = NULL;
  ...
  for (i = 0; i < DeviceRecordCacheSize; ++i) {
    DPRINTF_MB3 ("  Mb3CdXGetChildren loop 0x%x.\n", i);
    Status = Mb3CdXGetChildren (&(DeviceRecordCache[i]), ChildListHead);
    DPRINTF_MB3 ("  Mb3CdXGetChildren returned %r.\n", Status);
  }
...
SCT_BOOT_OPTION_PROTOCOL mMb3CdBootOptionProtocol = {
...
//
// FUNCTION NAME.
//      Mb3CdSaveDeviceRecord - Save DeviceRecordCache to variable.
//
// FUNCTIONAL DESCRIPTION.
...
//
// FUNCTION NAME.
//      Mb3CdFullScanPerformed - Callback when a full scan has been performed.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  DPRINTF_MB3 ("  Initialize gConnectedList of size 0x%x.\n", mCdConnectListArraySize);

  gConnectedList = AllocateZeroPool (mCdConnectListArraySize * sizeof (BOOLEAN));
  DPRINTF_MB3 (
    "  Allocated gConnectedList of size 0x%x @0x%x.\n",
    mCdConnectListArraySize * sizeof (BOOLEAN),
    gConnectedList
    );
  ...
  SataSearchNodeDevicePathSize = sizeof (mSataPortListArray) / sizeof (mSataPortListArray[0]);
  SearchNodeDevicePathArraySize = SataSearchNodeDevicePathSize + mCdEndNodeDevicePathsSize;
  ...
  for (i=0; i < SataSearchNodeDevicePathSize; ++i) {
    UnicodeSPrint (
      SataSearchNodeDevicePath + (i * BOP_SATA_DEVICE_LENGTH),
      BOP_SATA_DEVICE_LENGTH * sizeof (CHAR16),
      SataDevicePathTemplate,
      mSataPortListArray[i]
      );
  }
  ...
  for (i=0; i < mCdEndNodeDevicePathsSize; ++i) {
    SearchNodeDevicePathArray[i + SataSearchNodeDevicePathSize] = mCdEndNodeDevicePaths[i];
    DPRINTF_MB3 (
      "  Initialize SearchNode array[0x%x], value=[%s].\n",
      i + SataSearchNodeDevicePathSize,
...
//
// FUNCTION NAME.
//      InitializeBopOpticalDriveMb3 - Initialize the BopMb3Cd module.
//
// FUNCTIONAL DESCRIPTION.
...
//
// FUNCTION NAME.
//      Mb3CdXInsertDeviceRecord - Insert a DeviceRecord into cache.
//
// FUNCTIONAL DESCRIPTION.
...
//
// FUNCTION NAME.
//      Mb3CdXGetDevicePaths - Get device path from give device path pair.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  //
  // Connect all the device paths associated with this Mb3Cd Device.
  //

  Status = Mb3CdXSearchDevicePaths (ControllerIndex, SearchNodeIndex, &dp, &n);
  DPRINTF_MB3 ("  Mb3CdXSearchDevicePaths returned %r, %d device paths.\n", Status, n);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  DISPLAY_DEVICE_PATH_ARRAY (dp, n, L"  Mb3CdXSearchDevicePaths:");
  ...
  *NumberOfDevicePaths = 0;
  ...
  for (i = 0; i < n; i++, p = NextDevicePath (p)) {
    ...
    if (!Mb3CdIsDevicePathOpticalDrive (p)) {
      continue;
    }
...
//
// FUNCTION NAME.
//      Mb3CdXSearchDevicePaths - Get device path from give device path pair.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  if (gConnectedList[ControllerIndex] == FALSE) {
    ...
    Status = SctBdsLibConnectDevices (&(mCdConnectListArray[ControllerIndex]));
    if (EFI_ERROR (Status)) {
  ...
  ShadowOproms (&(mCdConnectListArray[ControllerIndex]));

  //
  // Search for devices that match this Mb3Cd criteria.
  //

  PathToSearch.ParentDevicePathText = mCdConnectListArray[ControllerIndex].TextDevicePath;
...
//
// FUNCTION NAME.
//      Mb3CdXGetChildren - Append Child Nodes to a Linked List.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  DPRINTF_MB3 ("Mb3CdXGetDevicePaths returned %r, %d entries.\n", Status, n);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  for (i = 0; i < n; i++, p = NextDevicePath (p)) {
    ...
    *q = AllocateZeroPool (sizeof (SCT_BOOT_OPTION_NODE));
    ...
    (*q)->Description = Mb3CdCreateDeviceDescription (p);
    DPRINTF_MB3 ("Mb3CdCreateDeviceDescription Done.\n");
...
//
// FUNCTION NAME.
//      Mb3CdCreateDeviceDescription - Create a string based on a device path.
//
// FUNCTIONAL DESCRIPTION.
...
//
// FUNCTION NAME.
//      Mb3CdIsDevicePathOpticalDrive - Check to see if a device path is an fixed disk.
//
// FUNCTIONAL DESCRIPTION.
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if ((BbsBcv.DeviceType == BBS_BEV_DEVICE) || (BbsBcv.DeviceType == BBS_CDROM)){
      return TRUE;
    }
    return FALSE;
  }
  ...
  if (!IsRemovableMediaDevicePath (DevicePath, NULL)) {
    DPRINTF_MB3 ("  Not removable device\n");
    return FALSE;
  }
  ...
  DPRINTF_MB3 (
    "GetUsbBbsDeviceTypeFromDevicePath: DeviceType = 0x%02x. Status = %r.\n",
    DeviceType,
    Status
    );
  ...
  if (EFI_ERROR (Status)) {
    return TRUE;
  } else if (DeviceType != BBS_TYPE_CDROM) {
    return FALSE;
  }
#endif // OPTION_SYSTEM_BOOT_MANAGER_LEGACY_BOOT
  return TRUE;
